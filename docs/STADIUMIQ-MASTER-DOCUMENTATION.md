...
verified against the working tree at the time of writing. The per-module reference (purpose,
problem-statement alignment, IO, key files, assumptions) lives in **§4** of this document; a
Word-format copy of this documentation ships alongside it as
`docs/StadiumIQ-Master-Documentation.docx`.
...
The GenAI layer runs Google `gemini-3.1-flash-lite` (tool calling, native structured JSON output,
and vision for ticket scanning), configured through `.env` behind a provider-agnostic adapter.
See §5.0.
...
| LLM narrates, never invents numbers | `lib/ai/copilot.ts` | `getForecastBrief` computes `findPeakCrush` / `forecastAt` first, then instructs the model it is "strictly forbidden from inventing or changing any numbers". |
...
| Structured LLM output contract | `lib/ai/agents.ts` | `assistantResponseSchema` (zod) parses `{ message, language, mapActions[], alertLevel, meta? }`. Enforced at parse time, with a salvage path (`buildLenientFallback`). |
...
| `docs/` | This file (canonical) plus the Word-format copy. |
...
`accessible: false` (`tests/venue.test.ts`).
...
- **Assumptions and limits:** Planning loop is capped at 2 tool round trips, then a forced final answer with no tools. Client aborts after 15s (`lib/assistant/client.ts`). History is capped at 8 turns, 600 chars per turn.
...
- **Assumptions and limits:** `computeRoute` takes a pre-resolved destination zone id, not the raw `DestinationQuery` (documented deviation, `lib/engine/routing.ts` file header). Closed gates are hard-excluded as pass-through; congested gates are penalized 4x, not excluded.
...
- **Depends on:** M3.
- **Assumptions and limits:** Sensory filters are soft only, so a route is always returned. One-off filters from the LLM take per-field precedence over stored preferences (`lib/ai/tools.ts`).
...
- **Depends on:** M3, M4, F3.
- **Assumptions and limits:** An organizer-triggered SOS cannot be dismissed by the fan (`SOSOverlay.tsx`). SOS replaces the whole fan view (`app/page.tsx`).
...
- **Assumptions and limits:** The expensive cascade prediction is memoized on timeline only, so it does not re-run on every 1s clock tick (`Dashboard.tsx`).
...
- **Key files:** `app/page.tsx` (breakpoint state, floating trigger, bottom sheet), `components/AppShell.tsx`, `tests/mobile.test.tsx`.
- **Depends on:** M2, M4, F1.
...
- **Key files:** `lib/engine/staffPrepositioning.ts` (`recommendPrepositioning`, `DEFAULT_PREPOSITION_COUNT`), consumed in `app/api/copilot/route.ts`.
...
- **Key files:** `lib/engine/rootCause.ts` (`traceRootCause`), consumed in `app/api/copilot/route.ts` and `lib/engine/debriefData.ts`.
...
- **Key files:** `lib/engine/forecastConfidence.ts` (`computeConfidenceBand`), consumed in `app/api/copilot/route.ts`.
- **Depends on:** M7, M18.
...
- **F1 — Design System and App Shell.** `lib/theme/tokens.ts` (accent `#2563EB`), `lib/motion/transitions.ts`, `components/ui/*`, `components/AppShell.tsx`, `components/RoleToggle.tsx`, `components/A11yControls.tsx`.
- **F2 — Venue Graph and Data Model.** `lib/venue/venue.ts` (`ZONES`, `EDGES`, `POIS`, `GATES`, `TRANSIT_NODES`, `TIERS`, `FACILITY_SPECS`, `getZone`, `getEdgesFrom`, `getPoisNear`), `lib/types.ts`.
- **F3 — Simulation Engine.** `lib/simulation/engine.ts`, `timeline.ts`, `channel.ts`, `scenarios.ts`, `lib/store/simStore.ts`. See §7.
- **F4 — Provider-Agnostic AI Layer.** `lib/ai/*`. See §5.
...
| Reason | Evidence |
|---|---|
| Vision requires Gemini | `GroqClient.visionChat` throws `VisionUnsupportedError` (`lib/ai/client.ts`). `GeminiClient.supportsVision = true`. M12's ticket scan calls `visionChat`, so Groq cannot serve it. |
| Latency budget | The fan planning loop allows up to 2 tool round trips plus a forced final answer, each capped at `LLM_TIMEOUT_MS` (default 8000ms, `lib/ai/env.ts`), all inside a 15s client abort (`lib/assistant/client.ts`). A Flash-Lite tier model keeps a multi-call turn inside that budget. |
| Native structured output | The Gemini adapter requests the `{ message, language, mapActions, alertLevel }` contract at the API level via a `responseSchema` (`GEMINI_RESPONSE_SCHEMA`, built once at module load), rather than only asking for it in the prompt. Groq falls back to `response_format: { type: 'json_object' }`. |

The model id is not hardcoded anywhere. It is read once from `process.env` through the
zod-validated accessor in `lib/ai/env.ts` and passed verbatim to the selected adapter. Changing
`LLM_MODEL` or setting `LLM_PROVIDER=groq` swaps the model or the entire provider with no code
change. The guard test in `tests/ai/client.test.ts` (describe `provider-agnostic invariant`)
recursively scans `lib/`, `app/`, and `components/` and fails the suite if any provider or model
id literal is reintroduced into shipped source.
...
| `getPolicy` | `query` | Top 2 knowledge chunks | `retrieve` (RAG, §5.5) |
| `reportIncident` | `type`, `note` | `{ success, incident }` | Returned for the client to apply (M15/M16) |

Origin and destination zone ids are validated against `ZONES` before routing, so a hallucinated
id returns `{ error: 'invalid_zone_id' }` rather than reaching the engine. Unknown tool names
...
Enforcement path in `parseResponse`:
...
1. Strip every XML block delimiter used in any prompt template (`user_message`, `user_query`,
   `incident`, `assignment`, `stadium_snapshot`, `forecast_calculations`), open and close
   variants, case-insensitive, replaced with `[filtered]`.
...
(`agents.ts`), fan-context fields interpolated into the system message (`agents.ts`), organizer
queries and incident notes (`copilot.ts`), and incident id/zone/responder (`incidentReport.ts`).
System prompts additionally instruct the model that tagged content is untrusted, and
`VISION_TICKET_PROMPT` states that text visible in the image is untrusted data.
...
All paths under `lib/engine/`. Pure modules take state as arguments and import no store, React,
or network module. The three `*Service.ts` files are the store-coupled orchestration layer; see
`lib/engine/README.md`.

| Function | File | Signature (abridged) | Behaviour |
|---|---|---|---|
| `computeRoute` | `routing.ts` | `(origin, destZoneId, edges, zones, density, routedLoad, gateStatus, filters?, sensory?, prebuilt?) => RouteResult \| RouteError` | Dijkstra over a weighted graph using a binary min-heap, O((V+E) log V). Optionally re-runs a naive unweighted graph to explain which zones/gates were avoided. |
| `buildGraph` | `routing.ts` | `(edges, zones, filters, gateStatus, density, routedLoad, sensory?) => Graph` | Weight = baseWalkSec x congestionFactor, then sensory penalty, then 4x for congested gates and 3x for each matching soft filter. Closed gates and (if accessibleOnly) stairs edges are excluded. |
| `congestionFactor` | `routing.ts` | `(density, load, routedLoad?, gateId?) => number` | `1 + 2.5*d + routedLoadPenalty`, monotone in both, load capped. |
| `shortestDistance` | `routing.ts` | `(graph, from, to) => number` | Dijkstra cost only, no path reconstruction. |
| `sensoryPenalty` | `routing.ts` | `(edge, toZone, options) => number` | Adds 50% of baseWalkSec per matching sensory condition. |
| `resolveDestination` | `destinationResolver.ts` | `(query, origin, edges, zones, pois, poiStatus, gateStatus, routedLoad) => string \| ResolveError` | Ranks POIs or gates by distance plus routed load. Density deliberately excluded: congestion affects the route to a destination, not which destination is chosen. |
| `routedLoadPenalty` | `loadBalance.ts` | `(routedLoad, gateId) => number` | `0.15 * min(load, 10)`. |
| `decayRoutedLoad` | `loadBalance.ts` | `(routedLoad) => Record<string, number>` | Multiplicative 0.9 decay per tick. |
| `forecastAt` | `forecast.ts` | `(timeline, matchClockSec, lookaheadSec) => ForecastResult` | Nearest-frame lookup, or projection when no timeline exists. |
| `findPeakCrush` | `forecast.ts` | `(timeline, matchClockSec, windowSec) => PeakCrushResult` | Scans frames in the window for the maximum density and its top zones. |
| `assignResponder` | `dispatch.ts` | `(incident, responders, edges, zones, ...) => DispatchAssignment` | Filters by skill and availability, ranks by travel time, returns the best plus `predictedBreach`. |
| `isBreachPredicted` | `dispatch.ts` | `(etaSec, slaSec = 300) => boolean` | ETA over SLA. |
| `evaluateTriggers` | `alertTriage.ts` | `(input: TriageInput) => Alert[]` | Threshold rules over density, gate status, and match clock. |
| `detectBottlenecks` | `incentiveTriage.ts` | `(input) => string[]` | Returns zone ids above the bottleneck threshold. |
| `predictCascades` | `cascadePrediction.ts` | `(frames, edges) => Cascade[]` | Computes first threshold crossings per zone, then links a hotspot to upstream neighbours that crossed earlier within the lookback window. |
| `traceRootCause` | `rootCause.ts` | `(zoneId, history, gateStatus, incidents, edges) => RootCauseChain` | Walks the incoming-edge map backward from the congested zone to a gate closure, incident, or earlier hotspot. |
| `computeConfidenceBand` | `forecastConfidence.ts` | `(point, timeline, zoneId) => ConfidenceBand` | Samples timeline variance where available, else a heuristic spread. |
| `recommendPrepositioning` | `staffPrepositioning.ts` | `(hotspot, responders, edges) => PrepositionRecommendation[]` | Picks the nearest available responder and back-computes a depart-by time from predicted crossing minus travel time. |
| `computeSafestExit` | `safeExit.ts` | `(input: SafeExitInput) => SafeExitResult` | Lowest-risk open exit for the SOS overlay. |
| `computeFlowVectors` | `flowVectors.ts` | `(currentDensity, previousDensity, edges) => FlowVector[]` | Per-edge destination-density delta above `FLOW_THRESHOLD`; returns `[]` if either snapshot is missing. |
| `resolveOverriddenDensity` | `overrideDecay.ts` | `(override, simulated, nowMs) => number` | Holds the manual value 30s, then ramps back to simulated over 20s. |
| `aggregateDebriefData` | `debriefData.ts` | `(state, edges) => DebriefInput` | Top bottlenecks with root-cause chains plus incident response stats. |
| `prioritizeAccessibleFacilities` | `facilities.ts` | `(pois, fromZoneId, needsAccessible) => Poi[]` | Promotes an accessible variant when it is comparably close. |
| `sensoryToRouteFilters` | `sensoryFilters.ts` | `(sensory) => RouteFilters` | Maps stored preferences onto soft route filters. |
| `evaluateStressEscalation` | `stressEscalation.ts` | `(input) => Incident \| null` | Creates a deduplicated incident when distress is detected. |
| `spawnAgents` / `stepAgents` | `agentMotion.ts` | `(zones, density, seed, ...)` / `(agents, density, deltaMs)` | Seeded spawn and per-frame motion, capped at `MAX_AGENTS = 300`. |

Store-coupled orchestrators: `runAlertTriageService()` (`alertService.ts`),
`runIncentiveTriageService()` (`incentiveService.ts`), `computeServiceRoute()`
(`routingService.ts`).
...
**Tick model.** `simStore` runs `setInterval(tick, 1000)`, one tick per real second. Each tick
snapshots `previousDensity`, recomputes density, blends sensor influence, decays routed load, and
recomputes gate status.

Constants (`engine.ts`): `MATCH_START_SEC = -1800`, `FIRST_HALF_END_SEC = 2700`,
...
**Timeline generation.** `generateTimeline(zones, seed)` pre-computes one `DensityFrame` per 60
...
**Fan as sensor.** `heartbeat(zoneId)` posts a HEARTBEAT over the channel.
`pruneAndCountSessions` drops entries older than `SESSION_TTL_MS` and counts live sessions per
zone. `blendSensorInfluence(base, count)` mixes that count into density at `SENSOR_WEIGHT = 0.3`,
saturating at `SENSOR_SATURATION = 8`. More open fan tabs in a zone therefore raise its measured
density.

**God Mode hooks.** `applyScenario(patch, isGodMode?)` merges a `Partial<SimState>` and
broadcasts SCENARIO. `mergeStatePatch` applies it. Manual per-zone density edits are held in
`manualDensityOverrides` and resolved through M19's decay curve. `reset(zones, config)`
broadcasts RESET.

**Judge import.** `importDataset(dataset)` validates through `validateUploadDatasetObject` and
returns `{ ok: true } | { ok: false, error }`, then broadcasts IMPORT so every tab converges.

**Sequencer.** `startAutoSequencer(zones)` initialises M29 and broadcasts
`SEQUENCER_INIT { seed, sessionStartedAtMs }` once. Every tab derives phase and density locally
from that seed.
...
|---|---|
| Key handling | `LLM_API_KEY` is read only in `lib/ai/env.ts`, used only inside route handlers, and sent as a request header, never a query string. It is absent from the client bundle. `.env` is gitignored; `.env.example` holds placeholders only. |
...
| Rate limiting | `lib/server/rateLimit.ts` fixed-window limiter, 30 requests/60s per client plus a 300/60s global per-route backstop. Client identity comes only from a proxy-set header (`x-vercel-forwarded-for`, or `x-forwarded-for` when Cloud Run's trace header proves the platform set it); a bare untrusted `x-forwarded-for` is deliberately ignored so header forging cannot mint fresh buckets. |
...
| Injection defense | See §5.4. Plus zone-id allowlisting in `lib/ai/tools.ts` and whitelist extraction in `lib/ai/languageDetection.ts`. |
...
Runner: Vitest 4 with jsdom (`vitest.config.ts`, setup `tests/setup.ts`). Unit tests are
colocated with their source (`lib/engine/routing.test.ts`); cross-cutting tests live in `tests/`.
...
| AI layer | `tests/ai/{client,agents,tools,rag,routes}.test.ts`, `lib/ai/{copilot,incidentReport,languageDetection,stressDetection,sanitize,parseLlmJson}.test.ts` |
...
and drives the adapters directly. Higher-level tests mock `createClient`
(`lib/ai/copilot.test.ts`, `lib/ai/incidentReport.test.ts`) or the browser fetch client
(`tests/assistant/AssistantPanel.test.tsx` mocks `lib/assistant/client`).

**Invariant guard.** `tests/ai/client.test.ts`, describe `provider-agnostic invariant`,
recursively scans `lib/`, `app/`, and `components/` (excluding test files) and fails if any
provider or model id literal appears in shipped source.
...
| Area | Implementation |
|---|---|
| Keyboard | Every interactive map element (60 sections, 4 gates, 4 transit nodes, POI markers) has `role="button"`, `tabIndex={0}`, and an onKeyDown handler for Enter and Space (`components/StadiumMap.tsx`). Covered by `tests/StadiumMap.test.tsx`. |
...
| Reduced motion | `useReducedMotion` gates framer-motion animations, and `@media (prefers-reduced-motion: reduce)` disables the map shimmer and pulse rings. |
...
Known gap: contrast has been checked by calculation for the header text on the accent
background, not by an automated axe or Lighthouse sweep of every view.

## 11. Known Limitations and Assumptions

- **Two time models coexist.** `lib/simulation/engine.ts` defines a full match clock
  ...
  timeline. `app/api/debrief/route.ts` accepts either as a full-time signal. They are not unified.
- **`lib/engine/` is not uniformly pure.** `alertService.ts`, `incentiveService.ts`, and
...
- **A tool touches a store.** `computeRoute` in `lib/ai/tools.ts` calls
  `useSimStore.getState().incrementRoutedLoad`. Stores are per-process, so on the server this is
  a no-op and the client's own `routingService` accounting is the real source of truth. The call
  is effectively test-environment-only.
- **Prompt-injection defense is best-effort.** Tag stripping plus idiom patterns reduce the
  attack surface; they are not a guarantee. The structured output contract, tool gating, and
  zone-id allowlisting are what actually bound the blast radius.
- **RAG is keyword overlap, not semantic.** `lib/ai/rag.ts` scores by unique word presence. A
  query phrased differently from `data/knowledge.md` may retrieve nothing.
- **Rate limiting is per-instance.** `lib/server/rateLimit.ts` holds buckets in process memory.
  A multi-instance or serverless deployment would need a shared store.
- **BroadcastChannel is same-browser, same-origin.** "Multi-tab sync" is not multi-user sync.
  Two people on two devices do not share state.
- **No authentication and no database.** The role toggle is a UI control. All state is
  client-side and lost on reload, except the fan location in localStorage.
- **Vision depends on the provider.** `GroqClient.visionChat` throws `VisionUnsupportedError`.
  Nothing validates that the configured `LLM_MODEL` actually supports vision or tool calling.
- **Stress detection is English-only** keyword and punctuation matching
  (`lib/ai/stressDetection.ts`), which is a gap against the multilingual goal of M12.
- **Crowd agents are cosmetic and off by default** (`MAX_AGENTS = 300`,
  `simStore.showCrowdAgents`).
- **Simulated data throughout.** No real sensor, ticketing, or transit feed. Density comes from
  a seeded function of zone id and match clock.
...
followed by a fix pass and a full regression re-audit. Outcome at the end of that pass:
production build clean, all tests passing, and no provider or model id literals in shipped
source (enforced by the guard test in `tests/ai/client.test.ts`).

A follow-up audit pass converted this master documentation to markdown as the canonical in-repo
reference (previously only the `.docx` shipped, while README and source comments pointed at
markdown paths that did not exist), eliminated the remaining production lint errors
(`react-hooks/set-state-in-effect`), removed unused UI scaffolding (`components/ui/toast.tsx`,
`dialog.tsx`, `button.tsx`, `card.tsx`, `EmptyState.tsx` where unreferenced), consolidated
reduced-motion detection onto framer-motion's `useReducedMotion`, and added colocated unit tests
for `lib/ai/sanitize.ts` and `lib/ai/parseLlmJson.ts`.
